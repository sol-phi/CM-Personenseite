...
class ExportService
{
    ...
    {
        $standaloneView = TemplateUtility::getDefaultStandAloneView();
        $standaloneView->getRenderingContext()->getTemplatePaths()->setTemplatePathAndFilename(TemplateUtility::getTemplatePath($this->getRelativeTemplatePathAndFileName()));
    ...
    {
        $fieldList = [];
        if ($mails instanceof QueryResultInterface) {
            /** @var Mail $mail */
            $mail = $mails->getFirst();
            if ($mail !== null) {
